...
model_configs = [
    ...
    {"size": "lagre", "d_model": 1280, "d_ff": 5120, "num_layers": 36, "num_heads": 20},
...
def profile_one_train_step(model, x, y, vocab_size, tag: str = ""):
    """
    仅执行一次训练步, 并分别对forward / backward / optimizer step
    开启独立的内存历史记录与到处快照
    """
    model.train()
    ...
    # ------------- Forward -------------
    torch.cuda.synchronize()  # 强制 CPU 等待，直到 GPU 完成当前所有正在进行的任务
    torch.cuda.memory._record_memory_history( # 开启Pytorch GPU 显存分配记录仪
        max_entries=1_000_000, context='all', stacks='all'
    )
    torch.cuda.reset_peak_memory_stats()  # 重置 CUDA 显存的“峰值（Peak）”统计计数器。以便重新开始统计
    out = model(x)
    torch.cuda.synchronize()

    fwd_alloc = torch.cuda.max_memory_allocated()  # 显存逻辑占用 理想值
    fwd_res = torch.cuda.max_memory_reserved()  # 显存物理占用 现实值
    torch.cuda.memory._dump_snapshot(f"memory_forward{('_' + tag) if tag else ''}.pickle")
    torch.cuda.memory._record_memory_history(enabled=None)  # 关闭GPU 显存分配记录仪
    ...
    loss.backward()
    torch.cuda.synchronize()
    bwd_alloc = torch.cuda.max_memory_allocated()  # 显存逻辑占用 理想值
    bwd_res = torch.cuda.max_memory_reserved()  # 显存物理占用 现实值
...
def parse_args():
    ...
    parser.add_argument("--all", action="store_true", help="Run all predifined configurations")
    ...
    parser.add_argument("--mixed_percison", type=bool, default=False, help="Use mixed percision to run the model")
...
def main():
    ...
    use_mixed = args.mixed_percison
    # Load checkpoint if resume is enabled
    checkpoint_path = Path(args.checkpoint)
    res = []
    completed_tasks = set()

    # 断点继训 如果基准测试脚本中途崩溃（例如因为 OOM）或被手动停止，下次运行时可以自动跳过已经跑完的配置，只跑剩下的部分，避免重复浪费时间。
    if args.resume and checkpoint_path.exists():
        ...
        print(f"[Resume] Found {len(res)} completed benckmarks")
    ...
    print()
    # 创建一个上下文管理器 
    # torch.autocast(device_type=device, dtype=torch.bfloat16) 在这个管理器的范围内，PyTorch 会自动根据操作类型，决定使用低精度（如 BF16）还是高精度（FP32）。
    # nullcontext() 不使用混合精度 用空上下文
    ctx = torch.autocast(device_type=device, dtype=torch.bfloat16) if use_mixed else nullcontext()

    # 在这个上下文里 自动执行精度的转换
    with ctx:
        for config in run_configs:
            for mode in ['forward', 'forward_and_backward']:
                task_id = {
                    'size': config['size'],
                    'mode': mode,
